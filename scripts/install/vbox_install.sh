...
VBOX_SOURCE="deb [arch=$ARCH signed-by=$PUBKEY_FILE] https://download.virtualbox.org/virtualbox/debian $DISTR contrib"
PACKAGE="virtualbox"
APT_CMD="sudo apt update && sudo apt install $PACKAGE"

# Download VirtualBox signing key, dearmor (convert from b64) and save.
...
sudo apt-key del info@virtualbox.org 2>/dev/null
...
echo "Adding VirtualBox repository to $SOURCES_FILE..."
sudo echo "${VBOX_SOURCE}" > "$SOURCES_FILE"
